...
export async function generateTickets(fastify, reference) {
  ...
  // Firestore transaction atomically checks ticketGenerated + processingLock
  // and sets processingLock before any work begins.
  //   "already_generated" → return early with existing ticket data
  //   "locked"            → another request is mid-flight; reject with 409
  //   "claimed"           → we hold the lock, safe to continue
  ...
  // claimResult === "claimed" — lock is ours, proceed with generation
  ...
  // Only runs for references created by the agent app (isAgentSale === true —
  // see spotix-agent's POST /api/v1/agent/sale, which writes these fields).
  ...
  // For agent-facilitated sales, what lands here is the amount AFTER the
  // agent's incentive is removed — the incentive was the agent's cut, not
  // the booker's, so it never counted toward the booker's withdrawable
  // balance in the first place. See processAgentSale() below for the
  // agent-side ledger entry the incentive amount actually goes to.
...
// ─── Agent sale processing ───────────────────────────────────────────────────
// Runs once per successfully-paid agent-created reference (isAgentSale: true).
...
export function generateTicketId() {
  ...
  // return `SPTX-TX-${Math.floor(10000000 + Math.random() * 90000000)}-${Math.random().toString(36).substring(2, 4).toUpperCase()}`;
}